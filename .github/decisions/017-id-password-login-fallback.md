...
### 3. バリデーションポリシー（MVP 方針）

**username (`LoginRequest.username`)**:
- **最小長: 1文字**（空文字不可）
- **最大長: 72文字**（実用的上限。超過時は 422 を返す）
- `pydantic.Field(min_length=1, max_length=72)` で設定済み

**password (`LoginRequest.password`)**:
- **最小長: 1文字**（空文字不可）
- **最大長: 128文字**（PBKDF2 の DoS 耒性確保のため必須）
- `pydantic.Field(min_length=1, max_length=128)` で設定済み
- 複雑度要件（大文字・数字・記号の強制）: **なし**（OWASP 2021 は複雑度強制を非推奨）
- 本番化前に最小長チェックの強化要検討（OWASP 推奨 8 文字以上）

> ⚠️ MVP トレードオフ: 最小長 1 文字のため短いパスワードが許容される。本番サービス化時には最小長（8 文字以上）を追加すること（別 Issue 管理予定）。
